...
bool readModbus(double &flow, double &total) {
  ...
  uint8_t req[] = {0x01, 0x04, 0x00, 0x00, 0x00, 0x14, 0xF0, 0x05};
  ...
  if(buf[0] != 0x01 && buf[1] != 0x04) return false;
  float reg1 = bytesToFloat(&buf[7]);
  double reg9 = parseToDouble(&buf[19]);
  flow = reg1*3600.0;
  total = reg9;
...
void setup() {
  ...
  if(isnanf(lastNightTotal))lastNightTotal=0.0;
...
// ===============================================================================
void loop() {
  // prefs.putDouble("yesterday_total", 0);
  // prefs.putDouble("lastNightTotal", 43249.05);
  ...
  if(dailyTotal<0)dailyTotal=0.0;
  getRelayStatus();
