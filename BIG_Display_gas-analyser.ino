...
String customFooter = "SOARMLICH GAS SYSTEM";
int scrollX = WIDTH;
...
<form action="/save" method="POST">
  ...
  <label>Company Name:</label><input type="text" name="company" value="%COMPANY%">
  <label>Company Address :</label><input type="text" name="address" value="%ADDRESS%">
...
void updateBuffer() {
  ...
  display.setCursor(4, 20);  display.print("SOx");
  display.setCursor(56, 20); display.print(sox_val); 
  display.setCursor(96, 20); display.print("ppm");  

  display.setCursor(4, 36);  display.print("NOx");
  display.setCursor(56, 36); display.print(nox_val);
  display.setCursor(96, 36); display.print("ppm");

  display.setCursor(4, 52);  display.print("PM");
  display.setCursor(56, 52); display.print(pm_val);
  display.setCursor(102, 52); display.print("mg/nm3");    

  display.setCursor(scrollX, 68); display.print(companyName); 
  display.setCursor(scrollX, 84); display.print(companyAddress);  
...
display.drawRect(0, 0, 128, 96, 1);
display.setCursor(4, 4);
display.print("FW VER: V4.2-CLASSIC");

display.setCursor(4, 20);
display.print("Soarmlich P10");
...
if (millis() - bootTime > AUTO_REBOOT_INTERVAL) {
  ...
  display.setCursor(scrollX, 68);display.print("SYSTEM REBOOTING ........... ");  
  ...
  sox_val = String(sox, 1);
  nox_val = String(nox, 1);
  pm_val  = String(pm, 1);
...
if (millis() - lastScrollTime > 30) {
    scrollX--;
    int textWidth = customFooter.length() * 6;
...
Serial.println();
  ...
  if (len >= 23)
{
    sox = modbusFloat(&resp[3]);
    nox = modbusFloat(&resp[7]);
    pm  = modbusFloat(&resp[11]);
    

    Serial.println("--------------------------------");
  Serial.printf("sox : %.1f\n", sox);
  Serial.printf("nox  : %.1f\n", nox);
  Serial.printf("pm  : %.1f C\n", pm);
            ...
            for (int chunk_x = 0; chunk_x < 4; chunk_x++)
            {
                for (int chunk_y = 0; chunk_y < 4; chunk_y++)
                {
                    for (int x_pixel = 0; x_pixel < 8; x_pixel++)
                    {
                        ...
                        drawScanBuffer[row][bit_idx] =
                            (finalX >= 0 && finalX < WIDTH &&
                             y >= 0 && y < HEIGHT)
                                ? renderBuffer[finalX][y]
                                : 0;
