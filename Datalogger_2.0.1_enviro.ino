...
void frimwareUpdate(String binURL);
String serial_id = "";
...
const byte START_HIGH = 0x00;
const byte START_LOW  = 0x00;
...
void constructRequest(byte *frame) {
  frame[0] = SLAVE_ID;
  frame[1] = FUNC_READ;
  frame[2] = START_HIGH;
  frame[3] = START_LOW;
...
// ====================================================================
bool readModbus(double &flow, double &total) {

//  uint8_t req[8]={ 0x01, 0x03, 0x07, 0xd0, 0x00, 0x14, 0x45, 0x48}; 
    uint8_t req[8];
  ...
  float reg1 =bytesToInt(&buf[3])/1000.0;

int totalInt = bytesToInt(&buf[19]);
int  totalFrac = bytesToFloat(&buf[23]);

  flow = reg1;
  total = totalInt + totalFrac/1000.0;
...
String getTimestamp() {
  ...
  if (year < 2024 || year > 2035) {
    Serial.println("⚠ Invalid RTC time detected");
...
void setup() {
  ...
  digitalWrite(RELAY_PIN,LOW);
  Serial.println("Current Firmware: " + firmware_version);
  if(WiFi.status() == WL_CONNECTED){
    checkForUpdates();
  }
  // =======================Auto Reboot Setup ================================
  ...
  if(isnan(lastNightTotal))lastNightTotal=0;
  ...
  if(isnan(yesterday_total))yesterday_total=0.0;
   ...
   // ===== LOAD STORED RESET DATE =====
  resetDay   = prefs.getInt("resetDay",25);
  resetMonth = prefs.getInt("resetMonth",5);
  Serial.print("Loaded resetDay: "); Serial.println(resetDay);
  Serial.print("Loaded resetMonth: "); Serial.println(resetMonth);
  delay(1000);
...
void loop() {
  ...
  // prefs.putDouble("lastNightTotal",0);// <-- KEEP COMMENTED
  ...
  Serial.print("Today  Day ");Serial.println(day);
  Serial.print("Reset  Day ");Serial.println(resetDay);
  Serial.print("Today month ");Serial.println(month);
  Serial.print("Reset  month ");Serial.println(resetMonth);
  ...
  // SCADA‑compatible field names
  doc["Serial_no"] = serial_id;
  doc["Current_version"] = firmware_version;
  doc["sensor_type"] = "RS485_Enviro";
  ...
  if(ch2_status==1){
    ...
    ch2_final_value =  total-lastNightTotal;
  ...
  hmi.writeStr("Calibration.t9.txt", String(flow));
  ...
  if(minute%5 == 0 && second>= 0 && second<=10){
  String payload;
...
// ===== FINAL CORRECTED RESET =====
  if (month > resetMonth || (month == resetMonth && day > resetDay)) {
    // Save yesterday's total to flash AND update in-memory variable immediately
    prefs.putDouble("yesterday_total", dailyTotal);
    yesterday_total = dailyTotal;                    // ← PREVIOUSLY MISSING
    Serial.print("Yesterday total saved: "); Serial.println(dailyTotal);

    // Advance lastNightTotal only if the meter actually moved
    if (total > lastNightTotal) {
      lastNightTotal = total;                        // update in-memory
      prefs.putDouble("lastNightTotal", total);      // save to flash
      Serial.print("lastNightTotal updated: "); Serial.println(total);
    }

    // Reset daily total to zero immediately
    dailyTotal = 0.0;
    // Move the reset point to today
    resetDay = day;
    resetMonth = month;
    prefs.putInt("resetDay", day);
    prefs.putInt("resetMonth", month);
